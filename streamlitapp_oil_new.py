...
if show_corr:
    ...
    # Filters: Location and Type
    selected_locations = st.multiselect("Filter by Location:", df["Location"].unique())
...
if show_scatter:
    ...
    # Filters: Location and Type
    sel_locs = st.multiselect("Filter by Location:", df["Location"].unique(), key="scatter_loc")
    sel_types = st.multiselect("Filter by Type:", df["Type"].unique(), key="scatter_type")

    df_sc = df.copy()
    if sel_locs:
        df_sc = df_sc[df_sc["Location"].isin(sel_locs)]
    if sel_types:
    ...
    # Compute correlation on filtered subset

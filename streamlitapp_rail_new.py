...
st.sidebar.title("Rail Benchmarking Dashboard")
...
if show_dashboard:
    ...
    c4.metric("Avg Safety Incidents", f"{avg_safety:.1f}")
    c5.metric("Avg Carbon Intensity", f"{avg_carbon:.1f}" if carbon_col else "N/A")
    ...
    if location_col:
        ...
        region_counts = df[location_col].value_counts()
    ...
    # Avg CapEx by Region
    if location_col and "CapEx ($M)" in df.columns:
        st.subheader("Avg CapEx by Region")
        capex_by_region = df.groupby(location_col)["CapEx ($M)"].mean().sort_values(ascending=False)
    ...
    if industry_col:
        ...
        dm_counts = df[industry_col].value_counts()
    ...
    if "CapEx ($M)" in df.columns:
        ...
        st.table(top5_capex[[project_col, "CapEx ($M)", location_col, industry_col]].reset_index(drop=True))
    if "Safety Incidents" in df.columns:
        ...
        st.table(top5_safety[[project_col, "Safety Incidents", "CapEx ($M)", location_col]].reset_index(drop=True))
    if carbon_col:
        st.write(f"**Top 5 by {carbon_col}**")
        top5_carbon = df.sort_values(carbon_col, ascending=False).head(5)
        st.table(top5_carbon[[project_col, carbon_col, "CapEx ($M)", location_col]].reset_index(drop=True))

    st.markdown("---")

    # Mini Correlation Snapshot
...
if show_distributions:
    ...
    # Categorical pie chart (exclude Project ID)
    if pie_cols:
...
if show_corr:
    ...
    # Filters: Region (Location) and Delivery Model (Industry)
    selected_regions = st.multiselect("Filter by Region:", df[location_col].unique() if location_col else [])
    selected_models = st.multiselect("Filter by Delivery Model:", df[industry_col].unique() if industry_col else [])

    filtered_df = df.copy()
    if selected_regions:
        filtered_df = filtered_df[filtered_df[location_col].isin(selected_regions)]
    if selected_models:
        filtered_df = filtered_df[filtered_df[industry_col].isin(selected_models)]
...
if show_scatter:
    ...
    # Filters: Region and Delivery Model
    sel_regions = st.multiselect("Filter by Region:", df[location_col].unique() if location_col else [], key="scatter_region")
    sel_models = st.multiselect("Filter by Delivery Model:", df[industry_col].unique() if industry_col else [], key="scatter_model")

    df_sc = df.copy()
    if sel_regions:
        df_sc = df_sc[df_sc[location_col].isin(sel_regions)]
    if sel_models:
        df_sc = df_sc[df_sc[industry_col].isin(sel_models)]
    ...
    # Select a specific project to highlight
    available_projects = df_sc[project_col].unique().tolist()
    sel_project = st.selectbox("Highlight a specific Project ID (optional):", [""] + available_projects)

    fig4, ax4 = plt.subplots(figsize=(5, 4))
    ax4.scatter(df_sc[x_col], df_sc[y_col], alpha=0.3, color="gray")

    if sel_project:
        proj_row = df_sc[df_sc[project_col] == sel_project]
        if not proj_row.empty:
            ax4.scatter(proj_row[x_col], proj_row[y_col], color="red", s=100, label=sel_project)
            ax4.legend()
    ...
    # Recompute correlation on filtered subset
    ...
    if st.checkbox("Show scatter matrix for selected columns"):
        ...
        if len(cols_sel) >= 2:
            fig5 = scatter_matrix(df_sc[cols_sel], alpha=0.6, diagonal="hist", figsize=(8, 8))
            st.pyplot(plt.gcf())
...
if show_top10:
    ...
    ax7.barh(df_sorted[project_col].astype(str), df_sorted[metric], color="skyblue")
    ...
    if st.checkbox("Rank by composite score (CapEx × Safety Incidents)"):
        ...
        if "CapEx ($M)" in df_prime.columns and "Safety Incidents" in df_prime.columns:
            ...
            st.dataframe(df_cs[[project_col, "CapEx ($M)", "Safety Incidents", "Composite Score"]].reset_index(drop=True))

            fig8, ax8 = plt.subplots(figsize=(6, 4))
            ax8.barh(df_cs[project_col].astype(str), df_cs["Composite Score"], color="salmon")

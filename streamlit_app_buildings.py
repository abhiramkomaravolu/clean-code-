...
# Clean—(no special parsing needed; numeric_cols/cat_cols defined directly)
numeric_cols = ["GFA (m²)", "Height", "CapEx ($M)"]
cat_cols     = ["Project ID", "Type"]
...
show_scatter = st.sidebar.checkbox("4. Scatter Plots", False)
...
if show_dash:
    ...
    if "Type" in df.columns:
        ...
        ax_t.set_xlabel("Type"); ax_t.set_ylabel("Count"); ax_t.set_title("Projects per Type")
        plt.xticks(rotation=45, ha="right"); st.pyplot(fig_t)
...
if show_overview:
    ...
    if cat_cols:
        ...
        for c in cat_cols:
            st.write(f"**{c}**"); st.dataframe(df[c].value_counts().reset_index().rename(columns={"index": c, c: "Count"}))

# Panel 2: Distribution & Outliers
if show_dist:
    st.header("2. Distribution & Outlier Analysis")
    col = st.selectbox("Select numeric column:", numeric_cols)
    fig_hist, ax_hist = plt.subplots(figsize=(6, 4))
    ax_hist.hist(df[col].dropna(), bins=20, edgecolor="black")
    ax_hist.set_title(f"Histogram of {col}"); ax_hist.set_xlabel(col); ax_hist.set_ylabel("Frequency")
    st.pyplot(fig_hist)
    ...
    if st.checkbox("Show boxplot for all numeric"):
        ...
        ax_bp.set_title("Boxplot of All Numeric Columns"); st.pyplot(fig_bp)

    if cat_cols:
        st.subheader("Categorical Distribution (Pie)")
        ...
        ax_pie.set_title(f"{cat} Distribution"); st.pyplot(fig_pie)

# Panel 3: Correlation & Significance
if show_corr:
    st.header("3. Correlation & Significance")
    corr_matrix = df[numeric_cols].corr()
    st.write("### Pearson Correlation Matrix"); st.dataframe(corr_matrix)
    ...
    if st.checkbox("Show p-value matrix"):
        ...
        st.write("### P-Value Matrix"); st.dataframe(p_values)

    if st.checkbox("Show R² matrix"):
        r_squared = corr_matrix ** 2
        st.write("### R² (Correlation²) Matrix"); st.dataframe(r_squared)

# Panel 4: Pairwise Scatter Plots
if show_scatter:
    ...
    fig_sc2, ax_sc2 = plt.subplots(figsize=(5, 4))
    ax_sc2.scatter(df[x_col], df[y_col], alpha=0.6)
    ax_sc2.set_xlabel(x_col); ax_sc2.set_ylabel(y_col); ax_sc2.set_title(f"{x_col} vs. {y_col}")
    st.pyplot(fig_sc2)
    r_val, p_val = pearsonr(df[x_col].dropna(), df[y_col].dropna())
    st.write(f"r = {r_val:.2f}, p = {p_val:.3e}, R² = {r_val**2:.2f}")
...
# Panel 5: Normalization & Clustering
if show_norm:
    ...
    if st.checkbox("Show Z-score normalized data"):
        ...
        st.write("### First 5 rows of normalized data"); st.dataframe(df_norm.head())
        st.download_button("Download normalized CSV", df_norm.to_csv(index=False).encode("utf-8"), "buildings_data_normalized.csv", "text/csv")
    if st.checkbox("Run K-Means clustering"):
        ...
        ax_pca2.set_xlabel("PC 1"); ax_pca2.set_ylabel("PC 2"); ax_pca2.set_title("PCA by Cluster")
        st.pyplot(fig_pca2)

# Panel 6: Top 10 Projects
if show_top10:
    st.header("6. Top 10 Projects by Chosen Metric")
    metric = st.selectbox("Select metric:", numeric_cols, index=numeric_cols.index("CapEx ($M)") if "CapEx ($M)" in numeric_cols else 0)
    order = st.radio("Sort order:", ["Descending", "Ascending"])
    ascending = (order == "Ascending")
    top10 = df.sort_values(by=metric, ascending=ascending).head(10)
    st.write(f"Top 10 by **{metric}** ({order}):"); st.dataframe(top10.reset_index(drop=True))
    ...
    ax_t102.set_xlabel(metric); ax_t102.set_ylabel("Project ID"); ax_t102.invert_yaxis(); ax_t102.set_title(f"Top 10 by {metric}")
    st.pyplot(fig_t102)

# Panel 7: Download Reports
if show_dl:
